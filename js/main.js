...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const yearsSinceFounding = currentYear - 1947;

  document.querySelectorAll('.year-count').forEach(el => {
    el.textContent = yearsSinceFounding.toString();
  });

  const siteDescription = document.getElementById('siteDescription');
  if (siteDescription) {
    siteDescription.setAttribute('content', `Cachaça artesanal de alambique com ${yearsSinceFounding} anos de tradição em Piranga, Minas Gerais. Conheça Risadinha Prata, Risadinha Ouro e Velho Parol.`);
  }

  const currentYearItem = document.getElementById('currentYearItem');
  if (currentYearItem) {
    currentYearItem.textContent = currentYear.toString();
  }

  const yearsStatItem = document.getElementById('yearsStatItem');
  if (yearsStatItem) {
    yearsStatItem.setAttribute('data-target', yearsSinceFounding.toString());
  }
